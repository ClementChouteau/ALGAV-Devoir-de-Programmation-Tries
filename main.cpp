...
int main() {
    ...
    /// FUSION
    {
        ...
        for (auto i = 0; i < (long)mots.size()/2; i++)
            t1.AjoutMot(mots[i]);
        end_time =  clock();
        std::cout << "TriePatricia<char> (1/2) AjoutMot: " << t1.ComptageMots() << " " << (float( end_time - begin_time ) /  CLOCKS_PER_SEC)*1000 << " ms" << std::endl;
        ...
        for (auto i = mots.size()/2; i < mots.size(); i++)
            t2.AjoutMot(mots[i]);
        end_time =  clock();
        std::cout << "TriePatricia<char> (2/2) AjoutMot: " << t2.ComptageMots() << " " << (float( end_time - begin_time ) /  CLOCKS_PER_SEC)*1000 << " ms" << std::endl;

        begin_time =  clock();
        TriePatricia<char> t3 = Fusion(t1, t2);
        end_time =  clock();
        std::cout << "Fusion TriePatricia<char> (1/2) (2/2): " << t3.ComptageMots() << " " << (float( end_time - begin_time ) /  CLOCKS_PER_SEC)*1000 << " ms" << std::endl;
